...
export default {
  ...
  methods:{
    ...
    getCommentInLesson(idLecture){
      baseApi.get(`/api/comment/getCommentLecture/${idLecture}`)
          .then((comment) => {
    ...
    getCourses(courseId){
      baseApi.get(`/api/v1/courses/getCourseById/${courseId}`)
          .then(c => {
            this.courses = c.data
            this.lecture = this.courses.sections[0].lectures[0]
    ...
    viewLecture(lecture){
      ...
      this.resetForm()
      this.getCommentInLesson(lecture.id)
      console.log("Chọn lecture: ", lecture.id)
    },
    resetForm(){
      this.comment = {
        id: null,
